...
async function fetchIcon(entry: ManifestEntry): Promise<Buffer> {
  ...
  for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt += 1) {
    try {
      ...
      if (!isPng(bytes)) {
        throw new Error(
          `Response is not a PNG (${response.headers.get("content-type") ?? "unknown content type"}, ${bytes.length} bytes)`,
        );
      }
  ...
  throw new Error(
    `Failed to download ${entry.skillType}:${entry.skillId} from ${entry.sourceImageUrl}: ${String(lastError)}`,
  );
}
...
const classDownloads = downloads.filter(
  ({ entry }) => entry.skillType === "class",
);
const personalDownloads = downloads.filter(
  ({ entry }) => entry.skillType === "personal",
);
const totalBytes = downloads.reduce((total, download) => total + download.bytes.length, 0);

console.log(`Downloaded ${classDownloads.length} class-skill icons.`);
console.log(`Downloaded ${personalDownloads.length} personal-skill icons.`);
console.log(`Wrote ${downloads.length} PNG files (${totalBytes} bytes).`);
